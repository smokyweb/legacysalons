...
if ( ! class_exists( 'SPFTESTIMONIAL_Options' ) ) {
	...
	class SPFTESTIMONIAL_Options extends SPFTESTIMONIAL_Abstract {
		...
		public function ajax_save() {
			...
			if ( ! $result ) {
				wp_send_json_error( array( 'error' => esc_html__( 'Error while saving the changes.', 'testimonial-pro' ) ) );
			} else {
		...
		public function set_options( $ajax = false ) {
			...
			if ( wp_verify_nonce( $nonce, 'spftestimonial_options_nonce' ) ) {
				...
				if ( ! $ajax && ! empty( $response['spftestimonial_import_data'] ) ) {
					...
					$importing    = true;
					$this->notice = esc_html__( 'Settings successfully imported.', 'testimonial-pro' );
				...
				if ( ! empty( $transient['reset'] ) ) {
					...
					$this->notice = esc_html__( 'Default settings restored.', 'testimonial-pro' );
		...
		public function add_admin_menu() {
			...
			$menu_capability = apply_filters( 'sp_real_testimonial_ui_permission', 'manage_options' );
		...
		public function add_options_html() {
			...
			if ( $show_buttons ) {
				echo '<h1> <img src="' . SP_TPRO_URL . '/Admin/assets/images/icon.svg" alt="">' . wp_kses_post( $this->args['framework_title'] ) . '</h1>';
			} else {
				echo '<h1 class="export-import"><img src="' . SP_TPRO_URL . 'Admin/assets/images/import-export.svg">' . wp_kses_post( $this->args['framework_title'] ) . '</h1>';
			}
			echo '</div>';

			echo '<div class="spftestimonial-header-right">';
			echo ( $has_nav && $this->args['show_all_options'] ) ? '<div class="spftestimonial-expand-all" title="' . esc_html__( 'show all settings', 'testimonial-pro' ) . '"><i class="fas fa-outdent"></i></div>' : '';

			echo ( $this->args['show_search'] ) ? '<div class="spftestimonial-search"><input type="text" name="spftestimonial-search" placeholder="' . esc_html__( 'Search...', 'testimonial-pro' ) . '" autocomplete="off" /></div>' : '';
			if ( $show_buttons ) {
				echo '<div class="spftestimonial-buttons">';
				echo '<input type="submit" disabled name="' . esc_attr( $this->unique ) . '[_nonce][save]" class="button button-primary spftestimonial-top-save spftestimonial-save' . esc_attr( $ajax_class ) . '" value="' . esc_html__( 'Save Settings', 'testimonial-pro' ) . '" data-save="' . esc_html__( 'Saving...', 'testimonial-pro' ) . '">';
				echo ( $this->args['show_reset_section'] ) ? '<input type="submit" name="spftestimonial_transient[reset_section]" class="button button-secondary spftestimonial-reset-section spftestimonial-confirm" value="' . esc_html__( 'Reset tab', 'testimonial-pro' ) . '" data-confirm="' . esc_html__( 'Are you sure to reset all the settings of this tab?', 'testimonial-pro' ) . '">' : '';
				echo ( $this->args['show_reset_all'] ) ? '<input type="submit" name="spftestimonial_transient[reset]" class="button spftestimonial-warning-primary spftestimonial-reset-all spftestimonial-confirm" value="' . ( ( $this->args['show_reset_section'] ) ? esc_html__( 'Reset All', 'testimonial-pro' ) : esc_html__( 'Reset', 'testimonial-pro' ) ) . '" data-confirm="' . esc_html__( 'Are you sure you want to reset all settings to default values?', 'testimonial-pro' ) . '">' : '';
				echo '</div>';
			}
				...
				if ( ! empty( $section['fields'] ) ) {
					...
					foreach ( $section['fields'] as $field ) {
						...
						SPFTESTIMONIAL::field( $field, $value, $this->unique, 'options' );
								...
								echo '<div class="spftestimonial-no-option">' . esc_html__( 'No data available.', 'testimonial-pro' ) . '</div>';
			...
			if ( ! empty( $this->args['show_footer'] ) ) {
				...
				echo '<div class="spftestimonial-buttons">';
				echo '<input type="submit" name="spftestimonial_transient[save]" class="button button-primary spftestimonial-save' . esc_attr( $ajax_class ) . '" value="' . esc_html__( 'Save', 'testimonial-pro' ) . '" data-save="' . esc_html__( 'Saving...', 'testimonial-pro' ) . '">';
				echo ( $this->args['show_reset_section'] ) ? '<input type="submit" name="spftestimonial_transient[reset_section]" class="button button-secondary spftestimonial-reset-section spftestimonial-confirm" value="' . esc_html__( 'Reset tab', 'testimonial-pro' ) . '" data-confirm="' . esc_html__( 'Are you sure to reset all the settings of this tab?', 'testimonial-pro' ) . '">' : '';
				echo ( $this->args['show_reset_all'] ) ? '<input type="submit" name="spftestimonial_transient[reset]" class="button spftestimonial-warning-primary spftestimonial-reset-all spftestimonial-confirm" value="' . ( ( $this->args['show_reset_section'] ) ? esc_html__( 'Reset All', 'testimonial-pro' ) : esc_html__( 'Reset', 'testimonial-pro' ) ) . '" data-confirm="' . esc_html__( 'Are you sure you want to reset all settings to default values?', 'testimonial-pro' ) . '">' : '';
				echo '</div>';
