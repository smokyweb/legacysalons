 ...
 * @subpackage Testimonial_Pro/tp-metabox
 */

namespace ShapedPlugin\TestimonialPro\Admin\Views\Framework\Classes;
...
class SPFTESTIMONIAL {
	 ...
	 * @var string
	 */
	public static $version = SP_TPRO_VERSION;
	/**
	 * Dir.
	 *
	 * @var string
	...
	public static function register_text_domain() {
		load_plugin_textdomain( 'testimonial-pro', false, dirname( plugin_basename( __FILE__ ), 6 ) . '/languages/' );
	}
	...
	public static function include_plugin_url( $file ) {
		return esc_url( SP_TPRO_URL . 'Admin/Views/Framework' ) . '/' . ltrim( $file, '/' );
	}
			...
			wp_localize_script(
				...
				array(
					...
					'i18n'          => array(
						...
						'no_results_text' => esc_html__( 'No results found.', 'testimonial-pro' ),
					),
				)
			);
	...
	public static function field( $field = array(), $value = '', $unique = '', $where = '', $parent = '' ) {
		...
		if ( ! empty( $field['_notice'] ) ) {
			...
			$field            = array();
			$field['content'] = esc_html__( 'Oops! Not allowed.', 'testimonial-pro' ) . ' <strong>(' . $field_type . ')</strong>';
			$field['type']    = 'notice';
			$field['style']   = 'danger';
		...
		if ( ! empty( $field_type ) ) {
			...
			if ( class_exists( $classname ) ) {
				...
				echo '<p>' . esc_html__( 'Field not found!', 'testimonial-pro' ) . '</p>';
			}
		} else {
			echo '<p>' . esc_html__( 'Field not found!', 'testimonial-pro' ) . '</p>';
		}
