<div class="fl-color-picker<# if ( data.field.className ) { #> {{data.field.className}}<# } if ( data.field.show_reset ) { #> fl-color-picker-has-reset<# } #>">
	<# if ( ! data.deferRendering ) { #>
		...
		<# } #>
	<# } #>
	<input name="{{data.name}}" type="hidden" value="{{{data.value}}}" class="fl-color-picker-value <# if ( data.deferRendering ) { #>fl-react-color-picker-value<# } #>" />
	<# if ( ! data.deferRendering ) { #>
	<div class="fl-clear"></div>
	<# } else { #>
	<div class="picker-mount"></div>
	<# } #>
</div>
