...
foreach ( $list_items as $k => $item ) :
	...
	$heading_html = '';
	if ( 'basic' === $settings->list_layout ) {
		$list_icon    = $module->get_list_icon( false, $list_icon_default );
		$content_icon = $module->get_content_icon( $list_icon, $settings->list_icon_placement );
		$content_html = $module->get_content_html( $settings->content_tag, $item, $content_icon, $settings->list_icon_placement );
	} else {
