...
final class BB_PowerPack_Admin_Settings {
	...
	static public function init_hooks() {
		...
		if ( isset( $_GET['page'] ) && 'ppbb-settings' == $_GET['page'] ) {
			...
			if ( isset( $_GET['sl_activation'] ) && 'false' == $_GET['sl_activation'] ) {
				if ( isset( $_GET['message'] ) ) {
					self::$errors[] = esc_html( $_GET['message'] );
				} else {
					self::$errors[] = esc_html__( 'An error occurred while verifying your license key.', 'bb-powerpack' );
				}
			}
		...
		add_action( 'fl_builder_cache_cleared', __CLASS__ . '::clear_video_thumb_cache' );
	}
	...
	static public function register_routes() {
		...
		if ( 'get_templates_data' === $action ) {
			...
			// Sort by active templates.
			usort( $templates_data, function ($left, $right) {
	...
	static public function get_tabs() {
		return apply_filters( 'pp_admin_settings_tabs', array(
			...
			'modules'	=> array(
				...
				'icon'      => '<span class="dashicons dashicons-welcome-widgets-menus"></span>',
	...
	static public function render_top_nav() {
		...
		<ul class="pp-admin-settings-topbar-nav">
			...
			<li class="pp-admin-settings-topbar-nav-item">
				<a href="https://wpbeaveraddons.com/docs/" target="_blank">
					<span class="dashicons dashicons-editor-help"></span>
					<span><?php _e( 'Documentation', 'bb-powerpack' ); ?></span>
				</a>
			</li>
			<?php } ?>
			<?php if ( ! $remove_support_link ) { ?>
			<li class="pp-admin-settings-topbar-nav-item">
				<a href="https://wpbeaveraddons.com/contact/?refr=pp_admin_settings&time=<?php echo time(); ?>" target="_blank">
					<span class="dashicons dashicons-email"></span>
					<span><?php _e( 'Support', 'bb-powerpack' ); ?></span>
				</a>
			</li>
			<?php } ?>
		</ul>
	...
	static public function render_tabs( $current_tab ) {
		...
		foreach ( $sorted_data as $data ) {
			if ( $data['show'] ) {
				...
				$icon = isset( $data['icon'] ) ? $data['icon'] : '<span class="dashicons dashicons-admin-generic"></span>';
				?>
				<a href="<?php echo self::get_form_action( '&tab=' . $data['key'] ); ?>" class="nav-tab<?php echo ( $current_tab == $data['key'] ? ' nav-tab-active' : '' ); ?>" title="<?php echo esc_attr( $data['title'] ); ?>">
					<?php echo $icon . ' <span>' . $data['title'] . '</span>'; ?>
				</a>
				<?php
			}
		}
	...
	static public function render_update_message() {
		if ( ! empty( self::$errors ) ) {
			foreach ( self::$errors as $message ) {
				echo '<div class="alert alert-error floating">' . $message . '</div>';
			}
		} elseif ( ! empty( $_POST ) && ! isset( $_POST['email'] ) ) {
			echo '<div class="alert alert-success floating">' . esc_html__( 'Settings updated!', 'bb-powerpack' ) . '<span class="dismiss">+</span></div>';
		}
	}
	...
	static private function save_white_label() {
		...
		$accent_color           = isset( $_POST['ppwl_accent_color'] ) ? sanitize_hex_color( $_POST['ppwl_accent_color'] ) : '';
		...
		self::update_option( 'ppwl_accent_color', $accent_color );
		...
		if ( $hide_wl_form || $hide_plugin ) {
			self::add_error( sprintf( __( 'White Label settings have been saved. Please copy the reset URL and keep it safe. You will need it to reset the White Label settings in future: %s', 'bb-powerpack' ), '<pre>' . esc_html( pp_wl_get_reset_url() ) . '</pre>' ) );
		}
	}
	...
	*/
	static private function save_extensions() {
		if ( isset( $_POST['pp-extensions-nonce'] ) && wp_verify_nonce( $_POST['pp-extensions-nonce'], 'pp-extensions' ) ) {
			...
			do_action( 'pp_admin_settings_extensions_save' );
		}
	}
	...
	*/
	static public function update_branding( $all_plugins ) {
		...
		if ( ! isset( $all_plugins[ $pp_plugin_path ] ) ) {
			return $all_plugins;
		}
	...
	*/
	...
	static public function clear_video_thumb_cache() {
		delete_transient( 'pp_video_thumbnail_cache' );
	}
}
