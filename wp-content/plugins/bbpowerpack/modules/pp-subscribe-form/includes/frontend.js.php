;(function($) {
	if ( $('.fl-node-<?php echo $id; ?>').length === 0 ) {
		return;
	}
	...
	<?php if ( ! FLBuilderModel::is_builder_active() && 'fixed_bottom' == $settings->box_type ) { ?>
		var postId = $('.fl-module.fl-node-<?php echo $id; ?>').closest('.fl-builder-content').attr( 'data-post-id' ) || '<?php echo get_the_ID(); ?>';
		$('.fl-module.fl-node-<?php echo $id; ?>').wrap('<div class="fl-builder-content fl-builder-content-' + postId + ' pp-wrap-<?php echo $id; ?>" data-post-id="' + postId + '"></div>');
