...
class PPSubscribeFormModule extends FLBuilderModule {
	...
	public function submit()
	{
		...
		if ( $email && $node_id ) {
			...
			if ( ! $result['error'] ) {
				...
				// Setup the success data.
					...
					$result['action'] = esc_attr( $settings->success_action );

					if ( 'message' == $settings->success_action ) {
						$result['message'] = $settings->success_message;
					}
					else {
						$result['url'] = esc_url( $settings->success_url );
					}
				}
		...
		$result = apply_filters( 'pp_subscribe_form_submission_result', $result, $settings, $email, $name );
