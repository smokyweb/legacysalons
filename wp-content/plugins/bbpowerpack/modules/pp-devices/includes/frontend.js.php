;(function($) {
	$(".fl-node-<?php echo $id; ?> .pp-device-orientation .fas.fa-mobile").on( 'click', function(e){
	...
	$(function() {
		...
		if ( 'yes' === $settings->stop_others ) {
			?>
				$(nodeclass + ' video').on('play', function() {
		...
		?>
		$video_obj.on('touchstart', function() {
			if ( ! $video_obj.hasClass( 'is-paused' ) ) {
				video.pause();
			} else {
				video.play();
			}
