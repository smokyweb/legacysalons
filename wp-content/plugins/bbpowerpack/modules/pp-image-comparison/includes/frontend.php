...
if ( isset( $settings->before_img ) && ! empty( $settings->before_img ) ) {
	...
	$before_img_attr = 'src="' . esc_url( $settings->before_img_src ) . '"';

	if ( isset( $before_alt ) && ! empty( $before_alt ) ) {
		$before_img_attr .= ' alt="' . esc_attr( $before_alt ) . '"';
	}
} else {
...
if ( isset( $settings->after_img ) && ! empty( $settings->after_img ) ) {
	...
	$after_img_attr = 'src="' . esc_url( $settings->after_img_src ) . '"';

	if ( isset( $after_alt ) && ! empty( $after_alt ) ) {
		$after_img_attr .= ' alt="' . esc_attr( $after_alt ) . '"';
	}
} else {
