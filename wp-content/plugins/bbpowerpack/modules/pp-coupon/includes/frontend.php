...
$coupon_class = 'pp-coupon-code pp-coupon-style-' . esc_attr( $settings->coupon_style ) . ' pp-coupon-icon-' . esc_attr( $settings->icon_position );
...
if ( isset( $settings->link_icon ) ) {
	$link_icon = '<i class="pp-coupon-link-icon ' . esc_attr( $settings->link_icon ) . '"></i>';
} else {
	$link_icon = '';
}
...
// link
$link = '<a class="pp-coupon-link" href="' . esc_url( do_shortcode( $settings->link_url ) ) . '" target="' . esc_attr( $settings->link_url_target ) . '" ' . $module->get_rel() . '>';
...
if ( isset( $settings->image_select ) && ! empty( $settings->image_select ) ) {
	...
	$image_attr = 'src="' . esc_url( $settings->image_select_src ) . '"';
