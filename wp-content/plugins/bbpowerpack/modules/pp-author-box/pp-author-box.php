	...
	public function render_link_tag( $link, $target = '_self', $tag_type = 'opening' ) {
		if ( 'opening' === $tag_type ) {
			if ( ! empty( $link ) ) {
				...
				?>
				<a href="<?php echo esc_url( do_shortcode( $link ) ); ?>" class="pp-authorbox--wrap" target="<?php echo esc_attr( $target ); ?>"<?php echo $nofollow; ?>>
				<?php
			} else {
				?>
			...
			<ul class="pp-author-social-links">
				<?php foreach ( $social_urls as $platform => $url ) {
					...
					<li class="pp-author-social-link social-platform-<?php echo $platform; ?>">
						<a href="<?php echo esc_url( $url ); ?>" target="_blank" ref="noopener nofollow">
