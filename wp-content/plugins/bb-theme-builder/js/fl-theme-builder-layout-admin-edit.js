( function( $ ) {
	...
	FLThemeBuilderLayoutAdminEdit = {
		...
		/**
		 * Initializes custom template rules.
		 *
		 * @since 1.1
		 * @access private
		 * @method _initCustomTemplateRules
		 */
		_initCustomTemplateRules: function()
		{
			var type 	= $( 'input[name=fl-theme-layout-type]' ).val(),
				wrap 	= $( '.fl-theme-builder-custom-template-rules' ),
				select 	= $( '.fl-theme-builder-custom-template-select' );
			...
			if ( 'singular' !== type ) {
				...
				select.multiselect( {
					selectAll: true,
					texts: {
						deselectAll     : FLThemeBuilderConfig.strings.deselectAll,
						noneSelected    : FLThemeBuilderConfig.strings.noneSelected,
						placeholder     : FLThemeBuilderConfig.strings.select,
						selectAll       : FLThemeBuilderConfig.strings.selectAll,
						selectedOptions : FLThemeBuilderConfig.strings.selected
					}
				} );
			}
		 },
