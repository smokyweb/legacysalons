...
final class FLThemeBuilderWooCommerceArchive {
	 ...
	 * @return void
	 */
	static public function posts_module_before_posts( $settings, $query = false ) {
		...
		if ( $is_woo_product && 'show' === $settings->woo_ordering ) {
			...
			if ( ! isset( $GLOBALS['woocommerce_loop'] ) ) {
				...
				$GLOBALS['woocommerce_loop']['total']        = $query->found_posts;
				$GLOBALS['woocommerce_loop']['total_pages']  = $query->max_num_pages;
				$GLOBALS['woocommerce_loop']['per_page']     = $query->get( 'posts_per_page' );
				$GLOBALS['woocommerce_loop']['current_page'] = max( 1, $query->get( 'paged', 1 ) );
				$force                                       = true;
			} else {
				$GLOBALS['woocommerce_loop']['current_page'] = max( 1, (int) FLBuilderLoop::get_paged() );
			}
	...
	static public function post_grid_after_content( $settings ) {
		...
		// Add to Cart Button
		if ( 'show' == $settings->woo_button && class_exists( 'FLPageDataWooCommerce' ) ) {
